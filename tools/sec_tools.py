...
class SEC10KTool(RagTool):
  ...
  args_schema: SkipValidation[Type[BaseModel]] = SEC10KToolSchema
  ...
  def get_10k_url_content(self, stock_name: str) -> Optional[str]:
    ...
    try:
      ...
      query = {
        "query": {
          "query_string": {"query": f'ticker:{stock_name} AND formType:"10-K"'}
        },
        "from": "0",
        "size": "1",
        "sort": [{"filedAt": {"order": "desc"}}],
      }
      filings = queryApi.get_filings(query)["filings"]  # type: ignore
      if len(filings) == 0:
...
class SEC10QTool(RagTool):
  ...
  args_schema: SkipValidation[Type[BaseModel]] = SEC10QToolSchema
  ...
  def get_10q_url_content(self, stock_name: str) -> Optional[str]:
    ...
    try:
      ...
      query = {
        "query": {
          "query_string": {"query": f'ticker:{stock_name} AND formType:"10-Q"'}
        },
        "from": "0",
        "size": "1",
        "sort": [{"filedAt": {"order": "desc"}}],
